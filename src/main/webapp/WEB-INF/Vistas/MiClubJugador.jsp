...
<%@ page import="vista_futbolDeBarrio.dtos.MiembroClubDto"%>
<%@ page import="vista_futbolDeBarrio.dtos.ClubDto"%>
...
<div class="container-fluid ">
							...
							<div class="row " style="background-color: #004000;">
								...
								<div class="col-sm-7 col-md-7 ">
									...
									<div class="row">
										<div class="col-sm-1 col-md-1 cabeceraAbajo "></div>
										<div class="col-sm-3 col-md-3 cabeceraAbajo ">
											<a href="${pageContext.request.contextPath}/jugador/misClubes"class="letraCabeceraAbajo" style="color: #d4af37;">MI
												CLUB</a>
										</div>
										<div class="col-sm-3 col-md-3 cabeceraAbajo ">
	...
	<main style="background-color: rgba(223, 234, 213, 0.5);">
		...
		<!-- Contenido normal de tu página -->
		<div id="misClubesContainer"></div>

		<!-- Modal al final del body -->
		<div id="modalVerClubMiClubJugador" class="modalMiClubJugador"
			style="display: none;">
			<div class="modal-contentMiClubJugador">
				<span class="cerrarModalMiClubJugador"
					onclick="cerrarModalVerClubMiClubJugador()">&times;</span>
				<h3 id="modalNombreClubMiClubJugador" class="mb-4"><b><u>Miembros del Club</u></b></h3>

				<div class="tabla-container">
					<table class="tablaDatosListaMarcadores tablaJugadores">
						<thead>
							<tr>
								<th>JUGADOR</th>
								<th>ALIAS</th>
						...
						<tbody id="tablaCuerpoJugadoresMiClubJugador">
							<!-- Se llenará con JS -->
						</tbody>
					</table>
				</div>

				<button class="botonCerrarModal"
					onclick="cerrarModalVerClubMiClubJugador()">Volver</button>
			</div>
		</div>
...
<a href="${pageContext.request.contextPath}/jugador/misClubes"
									style="color: white">Mis clubes</a>
...
<a href="${pageContext.request.contextPath}/jugador/eventos"
  ...
  <path
	...
	<script>
    // Guardar tipoUsuario y usuarioId en sessionStorage
    ...
    function cargarMisClubes() {
        ...
        fetch('${pageContext.request.contextPath}/jugador/misClubes?tipo=json')
            ...
            .then(data => {
                const container = document.getElementById('misClubesContainer');
                container.innerHTML = '';
                ...
                for (let i = 0; i < misClubes.length; i++) {
                    ...
                    html += '  <div class="izquierdaMiClubJugador">';
                    html += '      <img src="' + imagen + '" class="logoMiClubJugador" alt="Logo del club">';
                    html += '      <div class="infoMiClubJugador">';
                    html += '          <h4 class="nombreMiClubJugador">' + club.nombreClub + ' (' + club.abreviaturaClub + ')</h4>';
                    html += '          <p class="localidadMiClubJugador"><strong>Localidad:</strong> ' + club.localidadClub + ' - <strong>País:</strong> ' + club.paisClub + '</p>';
                    html += '          <p class="descripcionMiClubJugador">' + club.descripcionClub + '</p>';
                    html += '      </div>';
                    html += '  </div>';
                    html += '  <div class="botonesMiClubJugador">';
                    html += '      <button class="botonMiClubJugador botonVerMiClubJugador" onclick="verClubMiClubJugador(' + club.idClub + ')">Ver Club</button>';
                    html += '      <button class="botonMiClubJugador botonSalirMiClubJugador" onclick="salirClub(' + misClubes[i].idMiembroClub + ')">Salir del Club</button>';
                    html += '  </div>';
                    html += '</div>';
            ...
            .catch(error => {
                console.error(error);
                document.getElementById('misClubesContainer').innerHTML = '<p>Error al cargar los clubes.</p>';
            });
    }
    ...
    function verClubMiClubJugador(idClub) {
        fetch('${pageContext.request.contextPath}/jugador/misClubes?clubId=' + idClub + '&tipo=jugadores')
            ...
            .then(function(data) {
                var jugadores = data; // ya es un array
                var tbody = document.getElementById("tablaCuerpoJugadoresMiClubJugador");
                tbody.innerHTML = "";

                if (!jugadores || jugadores.length === 0) {
                    tbody.innerHTML = '<tr><td colspan="8">No hay jugadores en este club.</td></tr>';
                    return;
                }
                ...
                jugadores.forEach(j => {
                    var tr = '<tr>' +
                             ...
                             '<td>' + j.aliasJugador + '</td>' +
                             '<td>' + j.partidosJugadosGlobal + '</td>' +
                             '<td>' + j.partidosGanadosGlobal + '</td>' +
                             '<td>' + j.partidosPerdidosGlobal + '</td>' +
                             '<td>' + j.golesGlobal + '</td>' +
                             '<td>' + j.amarillasGlobal + '</td>' +
                             '<td>' + j.rojasGlobal + '</td>' +
                             '</tr>';
                    tbody.innerHTML += tr;
                });
