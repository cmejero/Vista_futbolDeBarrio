...
<header class="fixed-top">
  ...
  <path
	...
	<main style="background-color: rgba(223, 234, 213, 0.5);">
		...
		<div class="container-fluid" id="marcadorContainer">
			<div class="row">
				<div class="col-md-12 col-sm-12 col-12 mx-auto mt-3">
					<div class="row">
			...
			<div class="row">
				...
				<table class="tablaDatosListaMarcadores tablaClubes mb-4">
					<thead style="background-color: black;">
						<tr>
							<th style="border: 1.5px solid blue; width: 11%">POS</th>
							<th style="border: 1.5px solid blue; width: 30%">CLUB</th>
							<th style="border: 1.5px solid blue; width: 23%">LOCALIDAD</th>
							<th style="border: 1.5px solid blue; width: 6%">PJ</th>
							<th style="border: 1.5px solid blue; width: 6%">V</th>
							<th style="border: 1.5px solid blue; width: 6%">E</th>
							<th style="border: 1.5px solid blue; width: 6%">D</th>
							<th style="border: 1.5px solid blue; width: 6%">GF</th>
							<th style="border: 1.5px solid blue; width: 6%">GC</th>
		...
		<div class="container my-2" id="estadisticasPremiumContainer"
			...
			<div id="filtrosPremium" class="filaFiltrar"
				...
				<div class="filtroItem">
					<input type="text" id="buscarNombreEvento" class="inputFiltrar"
						placeholder="Buscar por nombre evento">
				</div>
				<input type="text" id="buscarPorVictoria" class="inputFiltrar"
					placeholder="Buscar por nº victorias">
			</div>
			<!-- Tabla de estadísticas de torneos premium -->
			<table class="tablaDatosListaMarcadores tablaPremium mb-4">
				<thead>
					<tr>
						<th style="width: 14%">Tipo Evento</th>
 ...
 Sevilla, España</pre>
  ...
  function aplicarFiltros(tablaBodyId, columnasInput) {
    columnasInput.forEach(({inputId, colIndex}) => {
      ...
      if (!input) return; // 👈 evita error si el input no existe
...
fetch(contextPath + "/jugadorEstadisticaGlobal", {
  ...
  .then(function(data) {
    ...
    // Ordenamos por porcentaje de victorias, luego goles a favor, luego goles en contra
    data.sort(function(a, b) {
    ...
    // Aplicar filtros y paginación si existen
    if (typeof filtrosClubes === "function") filtrosClubes();
    if (typeof paginarTabla === "function") paginarTabla("tablaClubes", 15);
  })
...
function crearFilaEstadisticasGlobalClubPremium(tipo, pj, gf, gc, ganados, perdidos) {
  ...
  var golesPorPartido = pj > 0 ? (gf / pj).toFixed(2) : "0.00";
...
function crearFilaEstadisticasTorneoClubPremium(tipo, torneo, pj, gf, gc, ganados, perdidos) {
  var fila = document.createElement("tr");
  var golesPorPartido = pj > 0 ? (gf / pj).toFixed(2) : "0.00";
  var pctVictorias = pj > 0 ? ((ganados / pj) * 100).toFixed(1) + "%" : "0%";
  var pctDerrotas = pj > 0 ? ((perdidos / pj) * 100).toFixed(1) + "%" : "0%";
