...
/**
 * Filtro de autoridad para controlar el acceso a endpoints según tipo de
 * usuario.
 *
 * <p>
 * - Rutas públicas: login, registro, información, CSS/Imágenes, etc. - Rutas
 * privadas: accesibles solo según tipo de usuario (jugador, club, instalación,
 * administrador). - En caso de acceso denegado: hace forward al JSP de login
 * con mensaje. - Previene que los usuarios escriban manualmente la URL de JSPs
 * privadas.
 */
@WebFilter("/*")
public class FiltroDeAutoridad implements Filter {
	 ...
	 * Controla: 1️⃣ Auto-login desde cookies si no hay sesión. 2️⃣ Redirección a
	 * raíz según tipo de usuario. 3️⃣ Permitir rutas públicas sin sesión. 4️⃣
	 * Control de permisos por tipo de usuario. 5️⃣ Forward a login JSP si acceso
	 * denegado.
	 */
	@Override
			...
			if (cookies != null) {
				for (Cookie c : cookies) {
					if ("tokenUsuario".equals(c.getName()))
						token = c.getValue();
					if ("tipoUsuario".equals(c.getName()))
						tipo = c.getValue();
				}
			}
	...
	private boolean esRutaPublica(String uri, String metodo) {
		...
		return uri.equals("/") || uri.contains("/quienesSomos") || uri.contains("/inicio") || uri.contains("/registrar")
				|| uri.contains("/marcadores") || uri.contains("/eventos") || uri.contains("/recuperarCuenta")
				|| uri.contains("/restablecerCuenta") || uri.contains("/login") ||

				uri.contains("/Css") || uri.contains("/Imagenes") || uri.contains("/detalleTorneo");
	}

	/**
	 * Determina si el tipo de usuario tiene permiso para acceder al endpoint
	 * privado
	 *
	 * @param uri         Ruta solicitada
	...
	private boolean tienePermisoSegunTipoUsuario(String uri, String tipoUsuario) {
		...
		switch (tipoUsuario) {
		case "jugador":
			return uri.contains("/jugador") || uri.contains("/jugador/eventos") || uri.contains("/jugador/misClubes")
					|| uri.contains("/jugador/marcadores") || uri.contains("/logout") || uri.contains("/pagoPremium");

		case "club":
			return uri.contains("/club") || uri.contains("/club/eventos") || uri.contains("/club/plantilla")
					|| uri.contains("/logout") || uri.contains("/pagoPremium");

		case "instalacion":
			return uri.contains("/instalacion") || uri.contains("/instalacion/eventos")
					|| uri.contains("/instalacion/torneo") || uri.contains("/instalacion/actaPartido")
					|| uri.contains("/logout") || uri.contains("/torneo/bracket");

		case "administrador":
			return uri.contains("/administrador") || uri.contains("/logout");
