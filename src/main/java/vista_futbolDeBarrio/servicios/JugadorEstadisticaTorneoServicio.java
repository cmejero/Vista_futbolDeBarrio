...
public class JugadorEstadisticaTorneoServicio {
	...
	public ArrayList<JugadorEstadisticaTorneoDto> obtenerJugadoresPorTorneo(Long torneoId) {
	    ArrayList<JugadorEstadisticaTorneoDto> lista = new ArrayList<>();
	    try {
	        // Endpoint nuevo de tu API que filtra por torneo
	        String urlApi = "http://localhost:9527/api/jugadorEstadisticaTorneo/torneo/" + torneoId;
	        URL url = new URL(urlApi);
	        HttpURLConnection conex = (HttpURLConnection) url.openConnection();
	        conex.setRequestMethod("GET");
	        conex.setRequestProperty("Accept", "application/json");
	        ...
	        if (responseCode == HttpURLConnection.HTTP_OK) {
	            ...
	            System.out.println("Error al obtener estadísticas de jugadores por torneo: " + responseCode);
	        }
	    } catch (Exception e) {
	        e.printStackTrace();
	    }
